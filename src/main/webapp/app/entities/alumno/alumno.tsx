...
export const Alumno = () => {
  ...
  const totalItems = useAppSelector(state => state.alumno.totalItems);
  ...
  return (
    <div>
      <h2 id="alumno-heading" data-cy="AlumnoHeading">
        <Translate contentKey="tallerConstruccionApp.alumno.home.title">Alumnos</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="tallerConstruccionApp.alumno.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/alumno/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="tallerConstruccionApp.alumno.home.createLabel">Create new Alumno</Translate>
          </Link>
        </div>
      </h2>
        ...
        {alumnoList && alumnoList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="tallerConstruccionApp.alumno.id">ID</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('id')} />
                </th>
                <th className="hand" onClick={sort('dni')}>
                  <Translate contentKey="tallerConstruccionApp.alumno.dni">Dni</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('dni')} />
                </th>
                ...
                <th className="hand" onClick={sort('tipoAlumno')}>
                  <Translate contentKey="tallerConstruccionApp.alumno.tipoAlumno">Tipo Alumno</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('tipoAlumno')} />
                </th>
                <th className="hand" onClick={sort('notaPromedio')}>
                  <Translate contentKey="tallerConstruccionApp.alumno.notaPromedio">Nota Promedio</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('notaPromedio')} />
                </th>
                <th />
              </tr>
            </thead>
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>
                    {alumno.fechaNacimiento ? (
                      <TextFormat type="date" value={alumno.fechaNacimiento} format={APP_LOCAL_DATE_FORMAT} />
                    ) : null}
                  </td>
                  <td>
                    <Translate contentKey={`tallerConstruccionApp.TipoAlumno.${alumno.tipoAlumno}`} />
                  </td>
                  <td>{alumno.notaPromedio}</td>
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/alumno/${alumno.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/alumno/${alumno.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="tallerConstruccionApp.alumno.home.notFound">No Alumnos found</Translate>
            </div>
          )
        )}
      </div>
