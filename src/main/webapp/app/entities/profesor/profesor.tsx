...
export const Profesor = () => {
  ...
  const totalItems = useAppSelector(state => state.profesor.totalItems);
  ...
  return (
    <div>
      <h2 id="profesor-heading" data-cy="ProfesorHeading">
        <Translate contentKey="tallerConstruccionApp.profesor.home.title">Profesors</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="tallerConstruccionApp.profesor.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/profesor/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="tallerConstruccionApp.profesor.home.createLabel">Create new Profesor</Translate>
          </Link>
        </div>
      </h2>
        ...
        {profesorList && profesorList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="tallerConstruccionApp.profesor.id">ID</Translate>{' '}
                ...
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/profesor/${profesor.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/profesor/${profesor.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="tallerConstruccionApp.profesor.home.notFound">No Profesors found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={profesorList && profesorList.length > 0 ? '' : 'd-none'}>
